...
public:
  ...
  void levyFlightMove(double maxWalkStep, size_t currGen, const std::vector<double>& upperBounds, const std::vector<double>& lowerBounds)
  {
    ...
    {
      _coords[i] = _coords[i] + levy * alpha;
      
      if(_coords[i] < lowerBounds[i])
      {
        _coords[i] = lowerBounds[i];
      }
      if(_coords[i] > upperBounds[i])
      {
        _coords[i] = upperBounds[i];
      }
    }
...
// [[Rcpp::export]]
Rcpp::List MothSearchOptCpp(SEXP optFunc, size_t popSize, int maxGeneration, double stopValue, double maxWalkStep, int OBLStopPercent, Rcpp::NumericVector rUpperBounds, Rcpp::NumericVector rLowerBounds, int numThreads)
{
